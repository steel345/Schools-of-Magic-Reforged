...
public class Air2Effect extends ImitationSpiteEffect {
   private boolean flip;
   private float startRot;

   public Air2Effect(Level world, double x, double y, double z, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.air_slow, world, x, y, z, 0.0, 0.0, 0.0, 1.0f, 0, 0, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public Air2Effect(Level world, double x, double y, double z, float scale, int age, int maxAge, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.air_slow, world, x, y, z, 0.0, 0.0, 0.0, scale, age, maxAge, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public Air2Effect(Level world, double x, double y, double z, double vX, double vY, double vZ, float scale, int age, int maxAge, Color color, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.air_slow, world, x, y, z, vX, vY, vZ, scale, age, maxAge, color);
      this.flip = flipped;
      this.startRot = startRot;
   }
   ...
   public void render(PoseStack pose, float partial) {
      ...
      if (this.world.isLoaded(blockpos) && !state.isSolidRender(this.world, blockpos)) {
         ...
         float alpha = ((1.0f - (float) (Math.max((double) this.age, 0.0) / (double) this.maxAge)) * 0.55f + 0.2f) * (float) this.color.getAlpha() / 255.0f;
