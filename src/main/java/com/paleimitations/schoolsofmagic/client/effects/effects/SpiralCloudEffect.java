...
public class SpiralCloudEffect extends ImitationSpiteEffect {
   private boolean flip;
   private float startRot;

   public SpiralCloudEffect(Level world, double x, double y, double z, boolean flipped, float startRot) {
      super(flipped ? ImitationSpriteLibrary.cloud2 : ImitationSpriteLibrary.cloud1, world, x, y, z, 0.0, 0.0, 0.0, 1.0f, 0, 0, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public SpiralCloudEffect(Level world, double x, double y, double z, float scale, int age, int maxAge, boolean flipped, float startRot) {
      super(flipped ? ImitationSpriteLibrary.cloud2 : ImitationSpriteLibrary.cloud1, world, x, y, z, 0.0, 0.0, 0.0, scale, age, maxAge, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public SpiralCloudEffect(Level world, double x, double y, double z, double vX, double vY, double vZ, float scale, int age, int maxAge, Color color, boolean flipped, float startRot) {
      super(flipped ? ImitationSpriteLibrary.cloud2 : ImitationSpriteLibrary.cloud1, world, x, y, z, vX, vY, vZ, scale, age, maxAge, color);
      this.flip = flipped;
      this.startRot = startRot;
   }
   ...
   public void render(PoseStack pose, float partial) {
      ...
      if (this.world.isLoaded(blockpos) && !state.isSolidRender(this.world, blockpos)) {
         double rotZ = (double) (this.flip ? -1 : 1) * 1.25 * Math.PI * (((double) this.age + (double) partial) / (double) this.maxAge) + (double) this.startRot;
