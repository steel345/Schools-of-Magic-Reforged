...
public class FlareEffect extends ImitationSpiteEffect {
   ...
   public FlareEffect(Level world, double x, double y, double z, float scale, int age, int maxAge) {
      super(ImitationSpriteLibrary.star, world, x, y, z, 0.0, 0.0, 0.0, scale, age, maxAge, Color.WHITE);
   }

   public FlareEffect(Level world, double x, double y, double z, double vX, double vY, double vZ, float scale, int age, int maxAge, Color color) {
   ...
   public void render(PoseStack pose, float partial) {
      ...
      ClientUtil.drawSprite(pose, iX, iY, iZ, 0.0, Math.toRadians(0.0), 0.0,
            uv.getA(), uv.getB(), this.sprite.getWidth(), this.sprite.getHeight(),
            this.scale, partial, r, g, b, alpha, light, false);
      ClientUtil.drawSprite(pose, iX, iY, iZ, 0.0, Math.toRadians(60.0), 0.0,
            uv.getA(), uv.getB(), this.sprite.getWidth(), this.sprite.getHeight(),
            this.scale, partial, r, g, b, alpha, light, false);
      ClientUtil.drawSprite(pose, iX, iY, iZ, 0.0, Math.toRadians(-60.0), 0.0,
            uv.getA(), uv.getB(), this.sprite.getWidth(), this.sprite.getHeight(),
