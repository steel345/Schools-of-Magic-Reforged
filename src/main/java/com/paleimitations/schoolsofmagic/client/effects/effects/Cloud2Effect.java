...
public class Cloud2Effect extends ImitationSpiteEffect {
   private boolean flip;
   private float startRot;

   public Cloud2Effect(Level world, double x, double y, double z, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.cloud4, world, x, y, z, 0.0, 0.0, 0.0, 1.0f, 0, 0, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public Cloud2Effect(Level world, double x, double y, double z, float scale, int age, int maxAge, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.cloud4, world, x, y, z, 0.0, 0.0, 0.0, scale, age, maxAge, Color.WHITE);
      this.flip = flipped;
      this.startRot = startRot;
   }

   public Cloud2Effect(Level world, double x, double y, double z, double vX, double vY, double vZ, float scale, int age, int maxAge, Color color, boolean flipped, float startRot) {
      super(ImitationSpriteLibrary.cloud4, world, x, y, z, vX, vY, vZ, scale, age, maxAge, color);
      this.flip = flipped;
      this.startRot = startRot;
   }
   ...
   public void render(PoseStack pose, float partial) {
      ...
      if (this.world.isLoaded(blockpos) && !state.isSolidRender(this.world, blockpos)) {
         double rotZ = (this.flip ? -0.25 : 0.25) * Math.PI * (((double) this.age + (double) partial) / (double) this.maxAge) + (double) this.startRot;
         float alpha = ((1.0f - (float) (Math.max((double) this.age, 0.0) / (double) this.maxAge)) * 0.2f + 0.1f) * (float) this.color.getAlpha() / 255.0f;
