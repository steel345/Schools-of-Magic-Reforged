...
public final class WandTextureCache {
   private static final ResourceLocation DEFAULT_FALLBACK = new ResourceLocation("som", "textures/entity/wand/core_ash.png");
   private static final Map<String, ResourceLocation> CACHE = new HashMap<>();

   private WandTextureCache() {}

   public static ResourceLocation getComposited(IWandData data) {
      if (data == null || data.getCoreType() == null || data.getHandleType() == null || data.getGemType() == null) {
      ...
      NativeImage base = loadLayer("core_" + core);
      if (base == null) return DEFAULT_FALLBACK;
      blendOnto(base, loadLayer("handle_" + handle));
      blendOnto(base, loadLayer("gem_" + gem));

      ResourceLocation id = new ResourceLocation("som", "dynamic/wand_" + key);
   ...
   private static NativeImage loadLayer(String name) {
      ResourceLocation loc = new ResourceLocation("som", "textures/entity/wand/" + name + ".png");
      try {
