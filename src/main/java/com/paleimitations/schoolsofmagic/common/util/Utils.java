...
public class Utils {
   ...
   public static Logger getLogger() {
      if (logger == null) {
         logger = LogManager.getFormatterLogger("som");
      }
      return logger;
   }
   ...
   public static boolean matches(List<ItemStack> inputs, List<ItemStack> recipeItems) {
      ...
      block0: for (ItemStack input : inputs) {
         for (ItemStack recipeItem : recipeItems) {
            if (!Utils.compareStacks(recipeItem, input)) continue;
   ...
   private static boolean compareStacks(ItemStack recipe, ItemStack supplied) {
      return recipe.is(supplied.getItem()) && recipe.getDamageValue() == supplied.getDamageValue() && NBTUtils.matchTag((Tag)recipe.getTag(), (Tag)supplied.getTag()) && supplied.getCount() >= recipe.getCount() || recipe.isEmpty() && supplied.isEmpty();
   }

   public static <E> List<E> getDifference(List<E> list1, List<E> list2) {
      ArrayList listOut = Lists.newArrayList();
      for (E element : list1) {
         if (list2.contains(element)) continue;
         listOut.add(element);
      }
      for (E element : list2) {
         if (list1.contains(element)) continue;
         listOut.add(element);
      }
      return listOut;
   }
   ...
   public static double getDistance(BlockPos pos1, BlockPos pos2) {
      double x1 = (double)pos1.getX() + 0.5;
      double y1 = (double)pos1.getY() + 0.5;
      double z1 = (double)pos1.getZ() + 0.5;
      double x2 = (double)pos2.getX() + 0.5;
      double y2 = (double)pos2.getY() + 0.5;
      double z2 = (double)pos2.getZ() + 0.5;
      double dx = x1 - x2;
      double dy = y1 - y2;
      double dz = z1 - z2;
      double d1 = Math.sqrt(Math.pow(dx, 2.0) + Math.pow(dz, 2.0));
      double d2 = Math.sqrt(Math.pow(dy, 2.0) + Math.pow(d1, 2.0));
      return d2;
   }
   ...
   public static boolean containsStacks(List<ItemStack> inputs, List<ItemStack> recipe) {
      for (ItemStack recipeItem : recipe) {
         if (!inputs.contains(recipeItem)) continue;
         inputs = Utils.createListWithoutItem(inputs, recipeItem);
      }
   ...
   public static <E> List<E> createListWithoutObject(List<E> list, E input) {
      ...
      for (E element : list) {
         if (input.equals(element)) continue;
         output.add(element);
      }
      return output;
   }

   public static List<ItemStack> createListWithoutItem(List<ItemStack> list, ItemStack input) {
      ArrayList output = Lists.newArrayList();
      for (ItemStack element : list) {
         if (ItemStack.isSameItem(input, input)) continue;
         output.add(element);
      }
      return output;
   }
   ...
   public static String getRandomName(Random random) {
      ...
      String firstName = name + Utils.getRandomEndName(random, gen);
      int m = random.nextInt(2);
      String last = Utils.getRandomStartName(random);
      for (int j = 0; j < m; ++j) {
         last = last + Utils.getRandomMiddleName(random);
      }
      String lastName = last + Utils.getRandomEndName(random, false);
