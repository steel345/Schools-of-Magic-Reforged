...
public class PotionPoisonThorned extends MobEffect {
   public PotionPoisonThorned(MobEffectCategory category, int color) {
      ...
      if (!world.isClientSide) {
         for (Entity e : ((ServerLevel)world).getAllEntities()) {
            if (e instanceof LivingEntity
               ...
               && ((LivingEntity)e).getEffect(PotionRegistry.poison_thorned.get()) == null
               && !(e instanceof EntityDryad)
               && !(e instanceof EntityNobleTree)) {
               e.hurt(world.damageSources().cactus(), 0.5F + 0.5F * (float)amplifier);
               ((LivingEntity)e).addEffect(new MobEffectInstance(MobEffects.POISON, 120, 0));
            }
         }
      }
   }
