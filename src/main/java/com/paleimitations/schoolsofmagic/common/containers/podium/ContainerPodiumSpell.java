...
public class ContainerPodiumSpell extends AbstractContainerMenu {
   protected final IItemHandler handler;
   protected final TileEntityPodium podium;

   public ContainerPodiumSpell(int id, Inventory playerInventory, FriendlyByteBuf buf) {
      this(id, playerInventory, resolve(playerInventory, buf.readBlockPos()));
   }

   public ContainerPodiumSpell(int id, Inventory playerInventory, TileEntityPodium podium) {
      super(MenuTypeRegistry.PODIUM_SPELL.get(), id);
      this.podium = podium;
      this.handler = podium.getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
      this.addSlot(new SlotSmall(this.handler, 0, 45, 105));
      this.addSlot(new SlotSmall(this.handler, 7, 196, 105));
      int xPos = 48;
      int yPos = 170;
      for (int y = 0; y < 3; ++y) {
         for (int x = 0; x < 9; ++x) {
   ...
   public ItemStack quickMoveStack(Player playerIn, int fromSlot) {
      ...
      if (slot != null && slot.hasItem()) {
         ...
         boolean failed = fromSlot < 2
            ? !this.moveItemStackTo(current, 2, 38, true)
            : !this.moveItemStackTo(current, 0, 2, false);
         if (failed) {
