...
public class ContainerMortNPest extends AbstractContainerMenu {
   private final TileEntityMortNPest te;
   private final IItemHandler handler;

   public ContainerMortNPest(int id, Inventory playerInventory, FriendlyByteBuf buf) {
      this(id, playerInventory, resolve(playerInventory, buf.readBlockPos()));
   }

   public ContainerMortNPest(int id, Inventory playerInventory, TileEntityMortNPest te) {
      super(MenuTypeRegistry.MORT_N_PEST.get(), id);
      this.te = te;
      this.handler = te.getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
      this.addSlot(new SlotSmall(this.handler, 0, 57, 30));
      this.addSlot(new SlotSmall(this.handler, 1, 103, 30));
      int xPos = 8;
      int yPos = 109;
      for (int y = 0; y < 3; ++y) {
         for (int x = 0; x < 9; ++x) {
   ...
   private static TileEntityMortNPest resolve(Inventory inv, BlockPos pos) {
      return (TileEntityMortNPest) inv.player.level().getBlockEntity(pos);
   }

   public TileEntityMortNPest getTile() {
      return this.te;
   }
