...
public class ContainerPodiumNote extends AbstractContainerMenu {
   protected final IItemHandler handler;
   protected final TileEntityPodium podium;

   public ContainerPodiumNote(int id, Inventory playerInventory, FriendlyByteBuf buf) {
      this(id, playerInventory, resolve(playerInventory, buf.readBlockPos()));
   }

   public ContainerPodiumNote(int id, Inventory playerInventory, TileEntityPodium podium) {
      super(MenuTypeRegistry.PODIUM_NOTE.get(), id);
      this.podium = podium;
      this.handler = podium.getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
      this.addSlot(new SlotSmall(this.handler, 6, 114, 97));
      int xPos = 48;
      int yPos = 170;
      for (int y = 0; y < 3; ++y) {
         for (int x = 0; x < 9; ++x) {
