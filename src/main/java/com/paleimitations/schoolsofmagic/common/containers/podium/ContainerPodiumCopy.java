...
public class ContainerPodiumCopy extends AbstractContainerMenu {
   protected final IItemHandler handler;
   protected final TileEntityPodium podium;

   public ContainerPodiumCopy(int id, Inventory playerInventory, FriendlyByteBuf buf) {
      this(id, playerInventory, resolve(playerInventory, buf.readBlockPos()));
   }

   public ContainerPodiumCopy(int id, Inventory playerInventory, TileEntityPodium podium) {
      super(MenuTypeRegistry.PODIUM_COPY.get(), id);
      this.podium = podium;
      this.handler = podium.getCapability(ForgeCapabilities.ITEM_HANDLER).orElse(null);
      this.addSlot(new SlotSmall(this.handler, 0, 44, 105));
      this.addSlot(new SlotItemHandler(this.handler, 1, 195, 105));
      this.addSlot(new SlotItemHandler(this.handler, 2, 120, 24));
      this.addSlot(new SlotItemHandler(this.handler, 3, 120, 46));
      this.addSlot(new SlotItemHandler(this.handler, 4, 120, 68));
      int xPos = 48;
      int yPos = 170;
      for (int y = 0; y < 3; ++y) {
         for (int x = 0; x < 9; ++x) {
   ...
   public ItemStack quickMoveStack(Player playerIn, int fromSlot) {
      ...
      if (slot != null && slot.hasItem()) {
         ...
         boolean failed = fromSlot < 5
            ? !this.moveItemStackTo(current, 5, 41, true)
            : !this.moveItemStackTo(current, 0, 5, false);
         if (failed) {
