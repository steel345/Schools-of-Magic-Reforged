...
public class ContainerSqueakard extends AbstractContainerMenu {
   private static final EquipmentSlot[] VALID_EQUIPMENT_SLOTS = new EquipmentSlot[]{
      EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET
   };
   private final EntitySqueakard entity;
   private final Inventory playerInventory;
   private final Container squeakardInventory;

   public ContainerSqueakard(int id, Inventory playerInventory, FriendlyByteBuf buf) {
      this(id, playerInventory, (EntitySqueakard) playerInventory.player.level().getEntity(buf.readInt()));
   }

   public ContainerSqueakard(int id, Inventory playerInventory, final EntitySqueakard entity) {
      super(MenuTypeRegistry.SQUEAKARD.get(), id);
      this.entity = entity;
      this.playerInventory = playerInventory;
      this.squeakardInventory = entity.inventory;
      int xPosHand = 116;
      int yPosHand = 8;
      for (int y = 0; y < 6; ++y) {
         for (int x = 0; x < 6; ++x) {
            this.addSlot(new Slot(this.squeakardInventory, x + y * 6, xPosHand + x * 18, yPosHand + y * 18));
         }
      }
      for (int index = 0; index < 4; ++index) {
         final EquipmentSlot equipmentSlot = VALID_EQUIPMENT_SLOTS[index];
         this.addSlot(new Slot(this.squeakardInventory, 3 - index + 36, 8, 29 + index * 18) {
            @Override
   ...
   public EntitySqueakard getEntity() {
      return this.entity;
   }
