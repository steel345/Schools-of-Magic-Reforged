...
public class EntityAISpellFireBlast extends EntityAIUseSpell {
   public EntityAISpellFireBlast(EntityMagician magician) {
      super(magician);
   }
   ...
   protected void castSpell() {
      ...
      if ((entity = this.findEntityOnPath(vec3d1, vec3d)) != null && entity == this.magician.getTarget()) {
         this.knockBack(this.magician.getTarget(), this.magician, 1.5F, this.magician.getX() - this.magician.getTarget().getX(), this.magician.getZ() - this.magician.getTarget().getZ());
         this.magician.getTarget().setSecondsOnFire(5);
         this.magician.getTarget().hurt(this.magician.level().damageSources().mobAttack(this.magician), 4.0F);
      }
      for (int i = 0; i < 30; ++i) {
   ...
   public void shoot(double x, double y, double z, float velocity, float inaccuracy, RandomSource rand) {
      ...
      if (this.magician.level() instanceof net.minecraft.server.level.ServerLevel serverLevel) {
         serverLevel.sendParticles(com.paleimitations.schoolsofmagic.common.registries.ParticleTypeRegistry.FLAME.get(),
